...
module.exports = {
  ...
  handleType (typeName, nonScalars) {
    ...
    for (const nonScalar of nonScalars) {
      fs.writeFileSync(path + `/${nonScalar.toLowerCase()}.js`,
        `export function ${nonScalar}(_parent, _args, context) {};`
      );
    }

    fs.writeFileSync(path + "/index.js",
      `${nonScalars.map(nonScalar => `import { ${nonScalar} } from "./${nonScalar}";`).join("\n")}\n\nexport const ${typeName} = {\n${nonScalars.map(nonScalar => `\t${nonScalar},`).join("\n")}\n};`
    );
  }
};